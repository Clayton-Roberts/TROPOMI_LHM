...
def calculate_pixel_area(latitudes, longitudes):
    ...
    latitudes  = latitudes + latitudes[0]
...
def create_time_series(fitted_results):
    ...
    methane_load_df = pd.DataFrame(columns=('Date',
                                            ...
                                            'Augmented_methane_load',
                                            'Augmented_methane_load_precision'
                                            ))
    ...
    for date in tqdm(summary_df.Date, desc='Creating pixel-coverage time series'):
        ...
        for filename in tropomi_overpasses:
            ...
            pixel_corner_latitudes  = np.array(original_ch4_file.groups['PRODUCT'].groups['SUPPORT_DATA'].groups['GEOLOCATIONS'].variables['latitude_bounds'][0])
            pixel_corner_longitudes = np.array(original_ch4_file.groups['PRODUCT'].groups['SUPPORT_DATA'].groups['GEOLOCATIONS'].variables['longitude_bounds'][0])
                ...
                for j in range(original_ch4_file.groups['PRODUCT'].dimensions['ground_pixel'].size):
                    if (ct.STUDY_REGION['Permian_Basin'][2] < pixel_centre_latitudes[i, j] < ct.STUDY_REGION['Permian_Basin'][3]) and \
                        ...
                        if original_pixel_values[i, j] < 1e30:
                            pixel_value              = original_pixel_values[i, j]
                            delta_pixel_value        = pixel_value - noaa_background
                            pixel_value_error        = original_pixel_precisions[i, j]
                            dry_air_column_density   = dry_air_column_densities[i, j]  # [mol m^-2]
                            methane_column           = delta_pixel_value * 1e-9 * dry_air_column_density  # [mol m^-2], need the 1e-9 to convert out from ppbv
                            methane_column_precision = methane_column * np.sqrt((pixel_value_error / delta_pixel_value) ** 2
                                                                                + (dry_air_column_density_error / dry_air_column_density) ** 2)
                            if qa_values[i, j] >= 0.5:
                                qa_pixels += 1
                                qa_pixel_values.append(pixel_value)
                                qa_column_densities.append([methane_column, methane_column_precision])
                            else:
                                poor_pixels += 1
                                poor_pixel_values.append(pixel_value)
                            ...
                            delta_pixel_value        = pixel_value - noaa_background
                            pixel_value_error        = prediction_pixel_precisions[i, j]
                            dry_air_column_density   = dry_air_column_densities[i, j]  # [mol m^-2]
                            methane_column           = delta_pixel_value * 1e-9 * dry_air_column_density  # [mol m^-2], need the 1e-9 to convert out from ppbv
                            methane_column_precision = methane_column * np.sqrt((pixel_value_error / delta_pixel_value) ** 2
                                                                                + (dry_air_column_density_error / dry_air_column_density) ** 2)
                            prediction_pixels += 1
                            used_prediction_pixel_values.append(pixel_value)
                            used_prediction_column_densities.append([methane_column, methane_column_precision])
        ...
        qa_methane_load           = sum([ch4_column * 7000.**2 for
                                         ch4_column in np.array(qa_column_densities)[:, 0]]) # each pixel is 7km by 7km
        qa_methane_load_precision = np.sqrt(sum([(ch4_column_precision * 7000.**2)**2 for
                                                 ch4_column_precision in np.array(qa_column_densities)[:, 1]]))

        poor_pixel_methane_load           = sum([ch4_column * 7000. ** 2 for
                                                 ch4_column in np.array(poor_pixel_column_densities)[:, 0]])  # each pixel is 7km by 7km
        poor_pixel_methane_load_precision = np.sqrt(sum([(ch4_column_precision * 7000. ** 2) ** 2 for
                                                         ch4_column_precision in np.array(poor_pixel_column_densities)[:, 1]]))

        prediction_pixel_methane_load           = sum([ch4_column * 7000. ** 2 for
                                                       ch4_column in np.array(used_prediction_column_densities)[:, 0]])  # each pixel is 7km by 7km
        prediction_pixel_methane_load_precision = np.sqrt(sum([(ch4_column_precision * 7000. ** 2) ** 2 for
                                                               ch4_column_precision in np.array(used_prediction_column_densities)[:, 1]]))
        ...
        methane_load_df = methane_load_df.append({'Date': date,
                                                  ...
                                                  'QA_plus_poor_pixel_methane_load_precision': np.sqrt(qa_methane_load_precision**2
                                                                                                       + poor_pixel_methane_load_precision**2),
                                                  'Augmented_methane_load': (qa_methane_load + poor_pixel_methane_load + prediction_pixel_methane_load),
                                                  'Augmented_methane_load_precision': np.sqrt(qa_methane_load_precision**2
        from src import constants as ct
        import netCDF4 as nc4
        import numpy as np
        from pyproj import Geod
        ...
        def unpack_no2(filename):
            ...
            pixel_values = np.array(file.groups['PRODUCT'].variables['nitrogendioxide_tropospheric_column'])[0]
            pixel_precisions = \
            np.array(file.groups['PRODUCT'].variables['nitrogendioxide_tropospheric_column_precision'])[0]
        ...
        def reduce_no2(no2_pixel_values, no2_pixel_precisions, no2_pixel_centre_latitudes, no2_pixel_centre_longitudes,
            ...
            for i in range(no2_pixel_values.shape[0]):
                ...
                for j in range(no2_pixel_values.shape[1]):
                    # ... if the coordinates are located in the study region of interest...
                    if (ct.STUDY_REGION['Permian_Basin'][2] < no2_pixel_centre_latitudes[i, j] <
                        ct.STUDY_REGION['Permian_Basin'][3]) and \
                            (ct.STUDY_REGION['Permian_Basin'][0] < no2_pixel_centre_longitudes[i, j] <
                             ct.STUDY_REGION['Permian_Basin'][1]):
        ...
        def reduce_ch4(ch4_pixel_values, ch4_pixel_precisions, ch4_pixel_centre_latitudes, ch4_pixel_centre_longitudes,
            ...
            for i in range(ch4_pixel_values.shape[0]):
                ...
                for j in range(ch4_pixel_values.shape[1]):
                    # ... if the coordinates are located in the study region of interest...
                    if (ct.STUDY_REGION['Permian_Basin'][2] < ch4_pixel_centre_latitudes[i, j] <
                        ct.STUDY_REGION['Permian_Basin'][3]) and \
                            (ct.STUDY_REGION['Permian_Basin'][0] < ch4_pixel_centre_longitudes[i, j] <
                             ct.STUDY_REGION['Permian_Basin'][1]):
        ...
        def get_colocated_measurements(filename):
            ...
            # Reduce the NO2 data down to what is contained within the study area
            no2_pixel_values, no2_pixel_precisions, no2_pixel_centre_latitudes, no2_pixel_centre_longitudes \
                = reduce_no2(no2_pixel_values, no2_pixel_precisions, no2_pixel_centre_latitudes,
                             no2_pixel_centre_longitudes, no2_qa_values)
            ...
            # Reduce the CH4 data down to what is contained within the study area
            ch4_pixel_values, ch4_pixel_precisions, ch4_pixel_centre_latitudes, ch4_pixel_centre_longitudes \
                = reduce_ch4(ch4_pixel_values, ch4_pixel_precisions, ch4_pixel_centre_latitudes,
                             ch4_pixel_centre_longitudes, ch4_qa_values)
        ...
        def augment_data_rich_days(fitted_results):
            ...
            for date in ['2019-01-31']:  # tqdm(summary_df.index, desc='Augmenting observations for data-rich days'):
                ...
                for filename in tropomi_overpasses:
                    ...
                    product_group.createDimension('time', ch4_file.groups['PRODUCT'].dimensions['time'].size)
                    product_group.createDimension('scanline', ch4_file.groups['PRODUCT'].dimensions['scanline'].size)
                    product_group.createDimension('ground_pixel',
                                                  ch4_file.groups['PRODUCT'].dimensions['ground_pixel'].size)
                    product_group.createDimension('corner', ch4_file.groups['PRODUCT'].dimensions['corner'].size)
                    ...
                    # Interpolate the NO2 data at the locations of the CH4 pixels
                    interpolated_no2_pixel_values = griddata(
                        (no2_pixel_centre_longitudes.flatten(), no2_pixel_centre_latitudes.flatten()),
                        no2_pixel_values.flatten(),
                        (ch4_pixel_centre_longitudes, ch4_pixel_centre_latitudes),
                        method='linear')

                    interpolated_no2_pixel_value_precisions = griddata(
                        (no2_pixel_centre_longitudes.flatten(), no2_pixel_centre_latitudes.flatten()),
                        no2_pixel_precisions.flatten(),
                        (ch4_pixel_centre_longitudes, ch4_pixel_centre_latitudes),
                        method='linear')

                    interpolated_no2_qa_values = griddata(
                        (no2_pixel_centre_longitudes.flatten(), no2_pixel_centre_latitudes.flatten()),
                        no2_qa_values.flatten(),
                        (ch4_pixel_centre_longitudes, ch4_pixel_centre_latitudes),
                        method='linear')
                    ...
                    for i in range(ch4_file.groups['PRODUCT'].dimensions['scanline'].size):
                        for j in range(ch4_file.groups['PRODUCT'].dimensions['ground_pixel'].size):
                            if (ct.EXTENT['Permian_Basin'][2] < ch4_pixel_centre_latitudes[i, j] <
                                ct.EXTENT['Permian_Basin'][3]) and \
                                    (ct.EXTENT['Permian_Basin'][0] < ch4_pixel_centre_longitudes[i, j] <
                                     ct.EXTENT['Permian_Basin'][1]):
                                # Perform a prediction whenever there is an available 'good' NO2 pixel.
                                if interpolated_no2_qa_values[i, j] >= 0.75:
                                    obs_no2 = interpolated_no2_pixel_values[i, j] * 1e3  # Convert to mmol / m^2
                                    sigma_N = interpolated_no2_pixel_value_precisions[
                                                  i, j] * 1e3  # Convert to mmol / m^2
                                    prediction, precision = fitted_results.predict_ch4(obs_no2, sigma_N, day_id)
                                    augmented_methane_mixing_ratio[i, j] = prediction
                                    augmented_methane_mixing_ratio_precision[i, j] = precision
        ...
        def add_dry_air_column_densities(fitted_results):
            ...
            for date in tqdm(summary_df.Date, desc='Adding dry air column density at methane pixel locations'):
                ...
                for filename in tropomi_overpasses:
                    ...
                    # Open the augmented CH4 file containing all the predictions.
                    augmented_ch4_file = nc4.Dataset(ct.FILE_PREFIX + '/augmented_observations/'
                                                     + fitted_results.run_name + '/data_rich/' + filename, 'a',
                                                     format='NETCDF4')
                    # Access the reference observation time of the CH4 observations (seconds since 2010-01-01 00:00:00)
                    ch4_base_time = np.array(original_ch4_file.groups['PRODUCT'].variables['time'])[0]
                    # Access the timedelta of each scanline from the reference time (milliseconds since the reference time)
                    ...
                    dry_air_column_densities = np.full(
                        np.array(original_ch4_file.groups['PRODUCT'].variables['methane_mixing_ratio'])[0].shape, 1e32)

                    for i in range(original_ch4_file.groups['PRODUCT'].dimensions['scanline'].size):
                        for j in range(original_ch4_file.groups['PRODUCT'].dimensions['ground_pixel'].size):
                            if (ct.EXTENT['Permian_Basin'][2] < pixel_centre_latitudes[i, j] <
                                ct.EXTENT['Permian_Basin'][
                                    3]) and \
                                    (ct.EXTENT['Permian_Basin'][0] < pixel_centre_longitudes[i, j] <
                                     ct.EXTENT['Permian_Basin'][1]):
                                # Construct the observation time in hours since Jan 1, 1900, 00:00:00
                                ch4_observation_datetime = datetime.datetime.strptime('20100101T000000',
                                                                                      '%Y%m%dT%H%M%S') \
                                                           + datetime.timedelta(seconds=int(ch4_base_time)) \
                                                           + datetime.timedelta(milliseconds=int(ch4_delta_times[i]))
                                observation_time_diff = ch4_observation_datetime - era5_base_time
        ...
        def calculate_dry_air_column_density_residuals(fitted_results):
            ...
            full_df.to_csv(ct.FILE_PREFIX + '/outputs/' + fitted_results.run_name + '/dry_air_column_densities.csv',
                           index=False)
        ...
        def create_time_series(fitted_results):
            ...
            for date in tqdm(summary_df.Date, desc='Creating pixel-coverage time series'):
                ...
                for filename in tropomi_overpasses:
                    ...
                    # Open the file of CH4 predictions.
                    prediction_ch4_file = nc4.Dataset(
                        ct.FILE_PREFIX + '/augmented_observations/' + fitted_results.run_name +
                        '/data_rich/' + filename, 'r')
                    ...
                    # Get the arrays of pixel corner coordinates, needed to calculate pixel areas.
                    pixel_corner_latitudes = np.array(
                        original_ch4_file.groups['PRODUCT'].groups['SUPPORT_DATA'].groups['GEOLOCATIONS'].variables[
                            'latitude_bounds'][0])
                    pixel_corner_longitudes = np.array(
                        original_ch4_file.groups['PRODUCT'].groups['SUPPORT_DATA'].groups['GEOLOCATIONS'].variables[
                            'longitude_bounds'][0])

                    original_pixel_values = \
                    np.array(original_ch4_file.groups['PRODUCT'].variables['methane_mixing_ratio'])[0]
                    original_pixel_precisions = \
                    np.array(original_ch4_file.groups['PRODUCT'].variables['methane_mixing_ratio_precision'])[0]
                    qa_values = np.array(original_ch4_file.groups['PRODUCT'].variables['qa_value'])[0]
                    prediction_pixel_values = \
                    np.array(prediction_ch4_file.groups['PRODUCT'].variables['methane_mixing_ratio'])[0]
                    prediction_pixel_precisions = \
                    np.array(prediction_ch4_file.groups['PRODUCT'].variables['methane_mixing_ratio_precision'])[0]
                    dry_air_column_densities = \
                    np.array(prediction_ch4_file.groups['PRODUCT'].variables['dry_air_column_density'])[0]
                        ...
                        for j in range(original_ch4_file.groups['PRODUCT'].dimensions['ground_pixel'].size):
                            if (ct.STUDY_REGION['Permian_Basin'][2] < pixel_centre_latitudes[i, j] <
                                ...
                                if original_pixel_values[i, j] < 1e30:
                                    ...
                                    dry_air_column_density = dry_air_column_densities[i, j]  # [mol m^-2]
                                    methane_column = delta_pixel_value * 1e-9 * dry_air_column_density  # [mol m^-2], need the 1e-9 to convert out from ppbv
                                    methane_column_precision = methane_column * np.sqrt(
                                        (pixel_value_error / delta_pixel_value) ** 2
                                        + (dry_air_column_density_error / dry_air_column_density) ** 2)
                                    if qa_values[i, j] >= 0.5:
                                        qa_pixels += 1
                                        qa_pixel_values.append(pixel_value)
                ...
                median_qa_value = np.median(qa_pixel_values)
                median_qa_plus_poor_pixel_value = np.median(qa_pixel_values + poor_pixel_values)
                median_augmented_coverage_pixel_value = np.median(
                    qa_pixel_values + poor_pixel_values + used_prediction_pixel_values)
                ...
                poor_pixel_methane_load = sum([ch4_column * 7000. ** 2 for
                                               ch4_column in
                                               np.array(poor_pixel_column_densities)[:, 0]])  # each pixel is 7km by 7km
                poor_pixel_methane_load_precision = np.sqrt(sum([(ch4_column_precision * 7000. ** 2) ** 2 for
                                                                 ch4_column_precision in
                                                                 np.array(poor_pixel_column_densities)[:, 1]]))

                prediction_pixel_methane_load = sum([ch4_column * 7000. ** 2 for
                                                     ch4_column in np.array(used_prediction_column_densities)[:,
                                                                   0]])  # each pixel is 7km by 7km
                prediction_pixel_methane_load_precision = np.sqrt(sum([(ch4_column_precision * 7000. ** 2) ** 2 for
                                                                       ch4_column_precision in
                                                                       np.array(used_prediction_column_densities)[:,
                                                                       1]]))
                ...
                methane_load_df = methane_load_df.append({'Date': date,
                                                          ...
                                                          'QA_methane_load_precision': qa_methane_load_precision,
                                                          'QA_plus_poor_pixel_methane_load': (
                                                                      qa_methane_load + poor_pixel_methane_load),
                                                          'QA_plus_poor_pixel_methane_load_precision': np.sqrt(
                                                              qa_methane_load_precision ** 2
                                                              + poor_pixel_methane_load_precision ** 2),
                                                          'Augmented_methane_load': (
                                                                      qa_methane_load + poor_pixel_methane_load + prediction_pixel_methane_load),
                                                          'Augmented_methane_load_precision': np.sqrt(
                                                              qa_methane_load_precision ** 2
                                                              + poor_pixel_methane_load_precision ** 2
                                                              + prediction_pixel_methane_load_precision ** 2)},
                                                         ignore_index=True)

            pixel_coverage_df.to_csv(ct.FILE_PREFIX + '/outputs/' + fitted_results.run_name + '/pixel_coverage.csv',
                                     index=False)
            median_pixel_value_df.to_csv(
                ct.FILE_PREFIX + '/outputs/' + fitted_results.run_name + '/median_pixel_value.csv', index=False)
            methane_load_df.to_csv(ct.FILE_PREFIX + '/outputs/' + fitted_results.run_name + '/methane_load.csv',
                                   index=False)
            + poor_pixel_methane_load_precision**2
                                                                                              + prediction_pixel_methane_load_precision**2)},
                                                 ignore_index=True)
