...
def generate_mu_and_Sigma():
    ...
    np.savetxt("test_suite/ground_truths/mu.csv", mu, delimiter=",")
    np.savetxt("test_suite/ground_truths/cov.csv", sigma, delimiter=",")
...
def generate_alphas_betas_and_gammas(days):
    ...
    mu    = np.loadtxt("test_suite/ground_truths/mu.csv", delimiter=",")
    sigma = np.loadtxt("test_suite/ground_truths/cov.csv", delimiter=",")
    ...
    df.to_csv('test_suite/ground_truths/alphas_betas_gammas.csv')
...
def generate_dataset():
    ...
    "ground_truths/alphas_betas_gammas.csv"'''

    df = pd.read_csv('test_suite/ground_truths/alphas_betas_gammas.csv', delimiter=",", header=0, index_col=0)

    with open('test_suite/ground_truths/dataset.csv', 'w') as csvfile:
...
def prepare_dataset_for_cmdstanpy():
    '''This function takes the "dataset.cvs" file located at "test_suite/ground_truths" and turns it into json
    that is suitable for usage by the cmdstanpy package (.csv files are unabled to be provided as data when we
    fit our models).'''

    df = pd.read_csv('test_suite/ground_truths/dataset.csv', delimiter=',', header=0, index_col=1)  # Indexing by Date instead of Day_ID
    ...
    with open('test_suite/data.json', 'w') as outfile:
